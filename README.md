...
| Minecraft (structure chests) | Village, mineshaft, stronghold, pyramid, vanilla-dungeon and Witchery chests draw every item from the chunk's shared populate `Random`, so contents depend on everything that consumed draws earlier in that chunk — mod `PopulateChunkEvent.Pre` handlers, an intersecting mineshaft, whichever village pieces were built first — and each of those is only as stable as the terrain reads behind it | Contents derived from the structure piece and the chest's position within it. The stock body still runs first, so every draw it would have made is still made and **zero blocks move** — measured: the block-differing chunk set is identical to the two-run noise floor. Pool, weights and roll range unchanged; only the RNG source moves. **Repeat fills now match vanilla.** Some generators fill one inventory more than once and the vanilla filler never clears, so stock accumulates the batches. Deriving each refill from the chest's position gave every batch the same seed, so each one wiped and reproduced the last. The refill rebuilds the whole stack per fill with a distinct seed per batch, and the batch index is counted **per site scope**, which is what makes the count route-pure. The two cases it separates are not the same thing: Thaumcraft hilltop circles have two deliberate call sites in one pass and keep two batches, while Village Names' biome pieces clip only their block placement and leave the fill unguarded, so every chunk box intersecting the piece refills a chest that already exists — 2 to 4 times, depending on what order the chunks populated in. Vanilla guards that twice over (`isVecInside` **and** `getBlock != Blocks.chest`) and fills a structure chest exactly once, so those rebuild one batch and repeat invocations are idempotent. Batch 0 is unchanged, so every singly-filled chest in the world is bit-identical to 0.10. **Balance at radius 60: 20 hilltop circles 142 -> 249 stacks; 11 Village Names sites 58 -> 34**, the latter down to the quantity vanilla's guard produces. An earlier release reported this as "31 chests, 169 -> 300 stacks, the quantity the generators asked for"; the village half of that was a mis-indexed counter restoring a quantity vanilla never produces, and is withdrawn |
