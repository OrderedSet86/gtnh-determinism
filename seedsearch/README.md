...
| **Vanilla `WorldGenDungeons` existence** | **yes — 2026-09-08** | no | Was route-dependent by TWO mechanisms, measured roughly equal: draw shift (`WorldGenLakes.generate` returns false on a world read before consuming a draw, moving all 8 attempt coordinates) and the scan itself (a chunk-crossing box read live). Fixed by forking the attempt coordinates (`RwgDungeonAttemptMixin`) AND reading the scan off `TerrainOracle` (`WorldGenDungeonsScanMixin`); either alone leaves the other. rows vs spiral 69/69, 0 diffs at radius 30. Stage 0 still cannot enumerate room positions. See `results/2026-09-08-vanilla-dungeon-determinism/`. NOTE: room count drops ~18%, unreviewed |
