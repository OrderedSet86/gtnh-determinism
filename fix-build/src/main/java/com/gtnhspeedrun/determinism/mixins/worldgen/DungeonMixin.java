...
public abstract class DungeonMixin {
    ...
    private int gtnhdet$fixedStats(IWorldEditor ed, Block block) {
        return 0;
    }

    /**
     * F5 fourth pass: hold {@link com.gtnhspeedrun.determinism.worldgen.PendingSlices}' atomic window for the whole of
    ...
    private void gtnhdet$openWindow(greymerk.roguelike.dungeon.settings.ISettings settings, int inX, int inZ,
        org.spongepowered.asm.mixin.injection.callback.CallbackInfo ci) {
        com.gtnhspeedrun.determinism.worldgen.SliceTrace.log("dungeon BEGIN at {},{}", inX, inZ);
        com.gtnhspeedrun.determinism.worldgen.PendingSlices.beginAtomic();
    }

    @org.spongepowered.asm.mixin.injection.Inject(
        method = "generate(Lgreymerk/roguelike/dungeon/settings/ISettings;II)V",
        at = @org.spongepowered.asm.mixin.injection.At("RETURN"))
    private void gtnhdet$closeWindow(greymerk.roguelike.dungeon.settings.ISettings settings, int inX, int inZ,
        org.spongepowered.asm.mixin.injection.callback.CallbackInfo ci) {
        com.gtnhspeedrun.determinism.worldgen.PendingSlices.endAtomic();
        com.gtnhspeedrun.determinism.worldgen.SliceTrace.log("dungeon END at {},{}", inX, inZ);
    }
    ...
    public boolean validLocation(Random rand, int x, int z) {
        ...
        final World world = this.editor instanceof WorldEditorAccess ? ((WorldEditorAccess) this.editor).gtnhdet$world()
            : null;
        ...
        if (gtnhdet$ground(TerrainOracle.block(world, x, upperLimit, z))) return false;

        int y = upperLimit;
        Block b;
        while (!gtnhdet$ground(b = TerrainOracle.block(world, x, y, z))) {
        ...
        for (int cx = x - 4; cx <= x + 4; cx++) {
            for (int cz = z - 4; cz <= z + 4; cz++) {
                if (gtnhdet$ground(TerrainOracle.block(world, cx, y + 4, cz))) return false;
            }
        }
        ...
        for (int cx = x - 4; cx <= x + 4; cx++) {
            for (int cz = z - 4; cz <= z + 4; cz++) {
                if (!gtnhdet$ground(TerrainOracle.block(world, cx, y - 3, cz))) airCount++;
    ...
    @Unique
    private static boolean gtnhdet$ground(Block b) {
