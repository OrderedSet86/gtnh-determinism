...
| Thaumcraft eldritch obelisks (crimson cult rings) — **new in 0.2** | Whether an obelisk exists at a given spot depended on which nearby candidate site your route generated first (a ~40-chunk-wide exclusion race) — the same seed could show or hide an obelisk per playthrough | Obelisk sites are a pure function of the seed; same seed → same obelisks for everyone, any route. **Positions shift once vs 0.1/stock**, and obelisks are noticeably rarer (the old race let every candidate retry; the deterministic rule doesn't — rebalance pending community feedback) |
...
- **Eldritch obelisk terrain test** — an obelisk site that wins its seed-pure lottery must still pass Thaumcraft's
  stock 5-column flatness/base test at generation time. For ~60% of sites at least one of those columns sits near a
  chunk border where a neighbor's trees can, in principle, flip the result by route. Not yet observed in testing
  (the one border site probed generated byte-identically in every order), but it's the known theoretical gap.
