#!/usr/bin/env bash
# THE canonical worldgenprobe builder. Every probe build and deploy goes through this script —
...
#
#   build-probe.sh [--deploy <server-dir> ...]
#
...
#
# Prints the built jar path + md5 on success. With --deploy, removes old worldgenprobe jars from
# each server dir's mods/ and copies the new jar by exact name, md5-verifying each copy.
set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
BUILD_DIR=$(cd "$SCRIPT_DIR/../probe-build" && pwd)
...
cd "$BUILD_DIR"
echo "building (clean, no configuration cache, JDK $GRADLE_JDK)…"
./gradlew --no-configuration-cache clean spotlessApply build -x test \
  -Dorg.gradle.java.home="$GRADLE_JDK" --console=plain 2>&1 | tail -3

JAR=$(ls -t build/libs/worldgenprobe-*.jar | grep -v -- '-dev\.jar$' | grep -v -- '-sources\.jar$' | head -1)
...
for res in mixins.worldgenprobe.json mixins.worldgenprobe.late.json; do
...
if [ "${1:-}" = "--deploy" ]; then
  ...
  for SRV in "$@"; do
    ...
    rm -f "$SRV/mods/"worldgenprobe-*.jar
    cp "$JAR" "$SRV/mods/"
